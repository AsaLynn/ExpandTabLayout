 ...
 * IUnreadMsg
 */
public class UnreadMsgUtils implements IUnreadMsg {

    public static void show(MsgView msgView, int num, @MsgLength int maxMsgLength) {
        if (msgView == null) {
            return;
        }
        ...
        if (num <= 0) {//圆点,设置默认宽高
            ...
            if (num > 0 && num < 10) {//圆
                ...
                msgView.setText(num + "");
            } else if (num >= maxMsgLength) {//圆角矩形,圆角是高度的一半,设置默认padding
                lp.width = RelativeLayout.LayoutParams.WRAP_CONTENT;
                msgView.setPadding((int) (6 * dm.density), 0, (int) (6 * dm.density), 0);
                if (maxMsgLength == TEN_LENGTH) {
                    msgView.setText("9+");
                } else if (maxMsgLength == HUNDRED_LENGTH) {
                    msgView.setText("99+");
                } else if (maxMsgLength == THOUSAND_LENGTH) {
                    msgView.setText("999+");
                } else if (maxMsgLength == TEN_THOUSAND_LENGTH) {

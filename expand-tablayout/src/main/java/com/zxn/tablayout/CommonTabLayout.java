...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    private Rect mUnselectIndicatorRect = new Rect();
    private GradientDrawable mIndicatorDrawable = new GradientDrawable();
    private List<GradientDrawable> mUnselectIndicatorDrawables = new ArrayList<>();
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mIndicatorColor = ta.getColor(R.styleable.CommonTabLayout_tl_indicator_color, Color.parseColor((mIndicatorStyle == STYLE_BLOCK || mIndicatorStyle == STYLE_BLOCK_SHADOW) ? "#4B6A87" : "#ffffff"));
        mUnselectIndicatorColor
                = ta.getColor(R.styleable.CommonTabLayout_tl_indicator_unselect_color, mIndicatorColor);

        mIndicatorHeight = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_height,
                dp2px(mIndicatorStyle == STYLE_TRIANGLE ? 4 : ((mIndicatorStyle == STYLE_BLOCK || mIndicatorStyle == STYLE_BLOCK_SHADOW) ? -1 : 2)));

        mIndicatorWidth = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_width, dp2px(mIndicatorStyle == STYLE_TRIANGLE ? 10 : -1));
        mIndicatorCornerRadius = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_corner_radius, dp2px((mIndicatorStyle == STYLE_BLOCK || mIndicatorStyle == STYLE_BLOCK_SHADOW) ? -1 : 0));
        mIndicatorMarginLeft = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_left, dp2px(0));
        mIndicatorMarginTop = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_top, dp2px((mIndicatorStyle == STYLE_BLOCK || mIndicatorStyle == STYLE_BLOCK_SHADOW) ? 7 : 0));
        mIndicatorMarginRight = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_right, dp2px(0));
        mIndicatorMarginBottom = ta.getDimension(R.styleable.CommonTabLayout_tl_indicator_margin_bottom, dp2px((mIndicatorStyle == STYLE_BLOCK || mIndicatorStyle == STYLE_BLOCK_SHADOW) ? 7 : 0));
        ...
        mIsDoubleTab = ta.getBoolean(R.styleable.CommonTabLayout_tl_indicator_is_double_tab, false);
        ta.recycle();
    }
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            if (mIconGravity == Gravity.LEFT) {
                ...
                tabView = mIsDoubleTab ? View.inflate(mContext, R.layout.layout_double_tab_top, null)
                        : View.inflate(mContext, R.layout.layout_tab_top, null);
            }

            mUnselectIndicatorDrawables.add(new GradientDrawable());
            tabView.setTag(i);
            addTab(i, tabView);
        }
    ...
    private void addTab(final int position, View tabView) {
        ...
        TextView tv_tab_content = (TextView) tabView.findViewById(R.id.tv_tab_content);
        if (mIsDoubleTab && null != tv_tab_content) {
            tv_tab_content.setText(mTabEntitys.get(position).getTabContent());
        }
...
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
            ...
            if (mIsDoubleTab) {
                TextView tv_tab_content = (TextView) tabView.findViewById(R.id.tv_tab_content);
                tv_tab_content.setTextColor(i == mCurrentTab ? mTextSelectColor : mTextUnselectColor);
            }
            if (mTextAllCaps) {
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (mIsDoubleTab) {
                TextView tv_tab_content = (TextView) tabView.findViewById(R.id.tv_tab_content);
                tv_tab_content.setTextColor(isSelect ? mTextSelectColor : mTextUnselectColor);
            }
            ImageView iv_tab_icon = (ImageView) tabView.findViewById(R.id.iv_tab_icon);
            CustomTabEntity tabEntity = mTabEntitys.get(i);
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIndicatorStyle == STYLE_TRIANGLE) {
        ...
        } else if (mIndicatorStyle == STYLE_BLOCK_SHADOW) {
            if (mIndicatorHeight < 0) {
                mIndicatorHeight = height - mIndicatorMarginTop - mIndicatorMarginBottom;
            }
            ...
            if (mIndicatorHeight > 0) {
                ...
                for (int i = 0; i < mUnselectIndicatorDrawables.size(); i++) {
                    GradientDrawable drawable = mUnselectIndicatorDrawables.get(i);
                    drawable.setColor(mUnselectIndicatorColor);
                    int width = mIndicatorRect.right - mIndicatorRect.left;

                    /*int left = (int) (paddingLeft + mIndicatorMarginLeft + width * i);
                    int right = (int) (paddingLeft + width * (i + 1) - mIndicatorMarginRight);
                    int top = (int) this.mIndicatorMarginTop;
                    int bottom = (int) (this.mIndicatorMarginTop + mIndicatorHeight);*/

                    View view = mTabsContainer.getChildAt(i);
                    int left = view.getLeft() + (view.getRight() - view.getLeft()) / 2 - width / 2;
                    int right = view.getLeft() + (view.getRight() - view.getLeft()) / 2 + width / 2;
                    int top = (int) this.mIndicatorMarginTop;
                    int bottom = (int) (this.mIndicatorMarginTop + mIndicatorHeight);

                    drawable.setBounds(left,//左
                            top,//上
                            right,//右
                            bottom);//下
